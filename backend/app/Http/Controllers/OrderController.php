...
class OrderController extends Controller
{
    ...
    {
        // Traemos TODAS las órdenes con relaciones
        $orders = Order::with(['client', 'items', 'status', 'creator'])->get();

        // Traemos TODOS los estados
        $statuses = OrderStatus::orderBy('id')->get();

        return $statuses->map(function ($status) use ($orders) {
            return [
                'id' => $status->id,
                'name' => $status->name,
                'orders' => $orders
